...
import com.luo.yupao.mapper.TeamMapper;
...
import com.luo.yupao.model.enums.TeamStatusEnum;
...
public class TeamServiceImpl extends ServiceImpl<TeamMapper, Team> implements TeamService {
    ...
    public boolean updateTeam(TeamUpdateRequest teamUpdateRequest,User loginUser) {
        ...
        if (oldTeam ==null){
            throw new BusinessException(ErrorCode.NULL_ERROR,"请求队伍为空");
        }
        // 3.只有管理员或者队伍创建人可以修改
        ...
        // 5.如果用户传入的新值和老值一致，就不用update了（可自行实现，降低数据库使用次数）
        // 6.更新成功
    ...
    public boolean joinTeam(TeamJoinRequest teamJoinRequest,User loginUser) {
        ...
        //只有一个线程能获取到锁
        RLock lock=redissonClient.getLock("yupi:join_team");
        try{
            //抢到锁 并执行
            while (true){
                if (lock.tryLock(0,-1, TimeUnit.MILLISECONDS)){
                    ...
                    long teamHasJoinNumTeam=userTeamService.count(userTeamQueryWrapper);
                    if (teamHasJoinNumTeam>0){
